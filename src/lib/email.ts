...
export class EmailService {
  static async sendConfirmationEmail(orderData: OrderWithItems, paymentResult: { paymentId: string; status: string; amount: number; currency: string }): Promise<{ success: boolean; messageId: string }> {
    try {
  ...
  static async sendEmail(emailData: { to: string; subject: string; html: string }): Promise<{ success: boolean; messageId: string }> {
    try {
  ...
  static async sendAlternativePaymentConfirmation(customerEmail: string, customerName: string, orders: OrderWithItems[], paymentMethod: string, totalAmount: number): Promise<{ success: boolean; messageId: string }> {
    try {
        ...
        <div style="margin-bottom: 30px; border: 1px solid #e0e0e0; border-radius: 8px; padding: 20px; background: #fafafa;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
            ...
            <tr style="font-weight: bold; background: white;">
              ...
              <td style="padding: 12px 8px; text-align: right;">${order.totalOrderAmount.toFixed(2)}</td>
            </tr>
          </table>
        </div>
      `;
